...
public class WeatherSochiServiceImpl implements WeatherSochiService {
    ...
    public SendMessage handle(Message message) {
        var weather = yandexApiFeignClient.getWeather("43.588348", "39.729996", true, yandexApiKey);
        String textWeather = getWeatherSochiNowFromYandexApiMessage(weather.getFact(), weather);
        return new SendMessage(String.valueOf(message.getChatId()),textWeather);
    }

    @Override
    public BotState getHandlerName() {
        return BotState.SOCHI;
    }

    private String getWeatherSochiNowFromYandexApiMessage(Fact fact, Weather weather) {
        StringBuilder weatherStringBuilder = new StringBuilder();
        weatherStringBuilder.append("Погода в Сочи сейчас:\n");
